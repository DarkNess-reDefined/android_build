...
function lunch()
{
    ...
    then
        ...
        T=$(gettop)
        cd $T > /dev/null
        vendor/dnd/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/dnd/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/dnd/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
